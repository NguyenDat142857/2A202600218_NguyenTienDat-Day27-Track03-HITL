...
Analyze the PR carefully and provide:
- summary
- review comments
- reasoning
- confidence score
...
def node_human_approval(state: ReviewState) -> dict:
    ...
    console.print(
        f"  [green]✓[/green] resumed with choice="
        f"{response['choice']}"
    )
...
def _post(state: ReviewState, label: str) -> str:
    ...
    try:
        ...
        console.print(
            f"  [green]✓[/green] posted comment to "
            f"{state['pr_url']}"
        )
...
def node_commit(state: ReviewState) -> dict:
    ...
    if state.get("human_choice") == "approve":
        return {
            "final_action": _post(state, "committed")
        }

    elif state.get("human_choice") == "edit":

        console.print(
            "[yellow]·[/yellow] reviewer requested edit"
        )

        return {
            "final_action": "needs_edit"
        }
...
def node_auto_approve(state):
    ...
    return {
        "final_action": _post(state, "auto_approved")
    }
...
def node_escalate(state):
    console.print(
        "[red]→ escalate[/red] "
        "[dim]exercise 3 will implement escalation[/dim]"
    )
...
def prompt_human(payload: dict) -> dict:
    ...
    while choice not in {"approve", "reject", "edit"}:
        ...
        choice = console.input(
            "\n[bold]Choice (approve/reject/edit)?[/bold] "
        ).strip().lower()
...
def main() -> None:
    ...
    console.print(
        f"[bold green]{result.get('final_action')}[/bold green]"
    )
